...
#include <math.h>
#include <ctype.h>
#include "subword_config.h"
#include "subword_vocab.h"
#include "utils.h"
    ...
    while ( fgets(sen, MAX_SENTENCE_LENGTH, fi) && line < max_line) {
      ...
      for (int i = gs->window - 1; i < sentence_length - gs->window + 1; i++) {
        ...
        for (int j = i - gs->window + 1; j < i + gs->window; j++) {
          ...
          long long l1 = words[j] * gs->layer1_size;
          if (gs->negative > 0) {
            // NS은 d==0 일때는 그 맥락 단어와 중심 단어를,
            // d==1 일때는 negative sample와 중심 단어를 연결한다.
            for (int d = 0; d < gs->negative + 1; d++) {
              ...
              float f = 0.0f;
              for (long long k = 0; k < gs->layer1_size; k++) {
                f += gs->layer1[k + l1] * gs->layer2[k + l2];
              }
