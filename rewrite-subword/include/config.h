...
typedef struct {
  ...
  int *subword_hash;
  long long subword_hash_size; // size of the hash table for subword
  vocab_word *subword_vocab; // subword vocabulary
  long long subword_size; // current subword size
  ...
  long long minx;
  long long maxx;
  long long sisg; // 0 -> 없음. 1 -> sisg-, 2 -> sisg+

  long long table_size;
  long long *random_table;
